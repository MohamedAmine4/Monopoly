...
import caseSansPropritaire.Luxe;
...
public class Plateau {
	...
	public Plateau() {
	   ...
	   // Méthode pour initialiser le plateau du Monopoly
	public void InitialiserPlateau() {
		 // Initialisation des cases du plateau
		plateau[0] = new Depart();
		plateau[1] = new Terrain("Boulvard de belleville", 60, 2, 50, 50, "marron");// (String nom,int prix,int
																					// loyer,int prixMaison,int
																					// prixHotel,String couleur)
	  ...
	  // Méthode pour ajouter les joueurs
	private void ajouterJoueurs() {
		   // Demande le nombre de joueurs
		Scanner scanner = new Scanner(System.in);
        int nombreJoueur = 0;

        // Boucle pour vérifier si l'entrée est un entier valide
		...
		if (nombreJoueur == 1)
			joueurs.add(new JoueurMonopoly("IA"));
		jouer();
	...
	public void jouer() {
		...
		while (!fintour) {
			...
			for (JoueurMonopoly j : joueurs) {
				...
				if (j.getArgent() <= 0) {
                    // Vérifie si le joueur n'a plus d'argent et quitte le jeu
		...
		if (position >= NB_CASES) {
			...
			if (ca instanceof ProprieterPublic) {
				...
				if (p.getProprietaire() == null) {
					...
					System.out.println("Vous Avez verser un montant de " + loyer + " £ de loyer a "
							+ prorietaire.getNom() + " votre solde Actuelle est " + j.getArgent());
				}
				...
				if (p.getPropritaire() == null)
					...
					if (p.getNbmaison() <= 4 && p.getNbHotel() == 0) {
						if (j.getArgent() >= p.getPrixMaison()) {
							System.out.println("vous voulez construire une maison a ce terrain (Oui,Non) avec prix= "
									+ p.getPrixMaison());
							String res;
							...
							if (res.equals("oui")) {
								...
								System.out.println(
										j.getNom() + " a acheté un maison .Votre solde actuelle est " + j.getArgent());
								p.setNbmaison(nbMaisonActuel + 1);
								switch (nbMaisonActuel) {
								case 1:
									p.setLoyer(5 * p.getLoyer());
									break;
								case 2:
									p.setLoyer(14 * p.getLoyer());
									break;
								case 3:
									p.setLoyer(36 * p.getLoyer());
									break;
								case 4:
									p.setLoyer(45 * p.getLoyer());
									break;
								default:
									break;
								}
								...
								System.out.println(
										j.getNom() + " n'a pas acheté des maisons pour le terrain " + p.getNom());
						...
						if (j.getArgent() >= p.getPrixHotel() && p.getNbHotel() == 0) {
							...
							System.out.println("vous voulez construire un hotel a ce terrain (Oui,Non) avec prix= "
									+ p.getPrixHotel());
							String res;
							if (j.getNom().equals("IA")) {
								res = j.ReponseIA();
							...
							if (res.equals("oui")) {
								...
								p.setNbHotel(1);
								System.out.println(
										j.getNom() + " a acheté un hotel .Votre solde actuelle est " + j.getArgent());
								p.setLoyer(50 * p.getLoyer());
							} else
								System.out
										.println(j.getNom() + " n'a pas acheté un hotel pour le terrain " + p.getNom());
				...
				if (j.getCarteChance().size() == 0 && j.getCarteCommunaute().size() == 0) {
					...
					j.setPosition(10);
				} else if (j.getCarteChance().size() != 0)
					j.getCarteChance().remove(0);
				else
					j.getCarteCommunaute().remove(0);
				...
				System.out.println("votre solde est " + j.getArgent());
			} else if (ca instanceof Luxe) {
				j.setArgent(j.getArgent() - 150);
				System.out.println("votre solde est " + j.getArgent());
			}

			else
				System.out.println("votre solde est " + j.getArgent()); // la case parc gratuit
